...
// Unexpected Keyboard: retain the most frequent matches across every spelling
// branch, instead of the first matches encountered in dictionary order.
static bool lpi_score_less(const LmaPsbItem &a, const LmaPsbItem &b) {
  return a.psb < b.psb || (a.psb == b.psb && a.id < b.id);
}

static size_t retain_lpi(LmaPsbItem *items, size_t capacity, size_t count,
                         const LmaPsbItem &item) {
  if (capacity == 0) return 0;
  if (count < capacity) {
    items[count++] = item;
    std::push_heap(items, items + count, lpi_score_less);
  } else if (lpi_score_less(item, items[0])) {
    std::pop_heap(items, items + count, lpi_score_less);
    items[count - 1] = item;
    std::push_heap(items, items + count, lpi_score_less);
  }
  return count;
...
bool DictTrie::load_dict_fd(int sys_fd, long start_offset,
  ...
  // Unexpected Keyboard: sys_fd is borrowed from AssetFileDescriptor.
  // fdopen takes ownership; using the Java-owned descriptor trips Android's
  // fdsan before the dictionary can be loaded. Own only a duplicate.
  int dictionary_fd = dup(sys_fd);
  if (dictionary_fd < 0)
    return false;
  FILE *fp = fdopen(dictionary_fd, "rb");
  if (NULL == fp) {
    close(dictionary_fd);
    return false;
  }
...
size_t DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], size_t lpi_max,
                                 LmaNodeLE0 *node, size_t lpi_num) {
  NGram& ngram = NGram::get_instance();
  for (size_t homo = 0; homo < (size_t)node->num_of_homo; homo++) {
    LmaPsbItem item = {};
    item.id = get_lemma_id(node->homo_idx_buf_off + homo);
    item.lma_len = 1;
    item.psb = static_cast<LmaScoreType>(ngram.get_uni_psb(item.id));
    lpi_num = retain_lpi(lpi_items, lpi_max, lpi_num, item);
  }
...
size_t DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], size_t lpi_max,
                                 ...
                                 uint16 lma_len, size_t lpi_num) {
  NGram& ngram = NGram::get_instance();
  for (size_t homo = 0; homo < (size_t)node->num_of_homo; homo++) {
    LmaPsbItem item = {};
    item.id = get_lemma_id(homo_buf_off + homo);
    item.lma_len = lma_len;
    item.psb = static_cast<LmaScoreType>(ngram.get_uni_psb(item.id));
    lpi_num = retain_lpi(lpi_items, lpi_max, lpi_num, item);
  }
  ...
  *lpi_num = 0;
  ...
  for (size_t son_pos = son_start; son_pos < son_end; son_pos++) {
    ...
    if (!cached) {
      ...
      if (need_lpi)
        *lpi_num = fill_lpi_buffer(lpi_items, lpi_max, son, *lpi_num);
    }
    ...
    if (son->spl_idx == id_start) {
      if (mile_stones_pos_ < kMaxMileStone &&
        ...
        // Unexpected Keyboard: expose a workspace capacity failure.
        fprintf(stderr, "Pinyin workspace exhausted: milestones=%u/%d marks=%u/%d\n",
                static_cast<unsigned>(mile_stones_pos_), kMaxMileStone,
                static_cast<unsigned>(parsing_marks_pos_), kMaxParsingMark);
      }
    }
    ...
    for (uint16 ext_pos = 0; ext_pos < ext_num; ext_pos++) {
      ...
      for (size_t son_pos = 0; son_pos < (size_t)node->num_of_son; son_pos++) {
        ...
        if (son->spl_idx >= id_start
          ...
          *lpi_num = fill_lpi_buffer(lpi_items, lpi_max, homo_buf_off, son, 2, *lpi_num);
    ...
    for (uint16 ext_pos = 0; ext_pos < ext_num; ext_pos++) {
      ...
      for (size_t son_pos = 0; son_pos < (size_t)node->num_of_son; son_pos++) {
        ...
        if (son->spl_idx >= id_start
            && son->spl_idx < id_start + id_num) {
          size_t homo_buf_off = get_homo_idx_buf_offset(son);
          *lpi_num = fill_lpi_buffer(lpi_items, lpi_max, homo_buf_off, son,
                                    dep->splids_extended + 1, *lpi_num);
...
size_t DictTrie::get_lpis(const uint16* splid_str, uint16 splid_str_len,
  ...
  if (splid_str_len == 0 || splid_str_len > kMaxLemmaSize || max_lma_buf == 0 || root_ == NULL)
    return 0;

  // Unexpected Keyboard: a large dictionary can have more than 200 matching
  // spelling nodes for initials such as "zg". Visit every matching branch,
  // then retain the best-scoring lemmas within the caller's result capacity.
  uint16 id_start = splid_str[0];
  uint16 id_num = spl_trie_->is_half_id(id_start)
      ? spl_trie_->half_to_full(id_start, &id_start) : 1;
  std::vector<LmaNodeLE0*> first;
  for (size_t pos = splid_le0_index_[id_start - kFullSplIdStart];
       pos < splid_le0_index_[id_start + id_num - kFullSplIdStart]; pos++)
    first.push_back(root_ + pos);
  size_t lma_num = 0;
  if (splid_str_len == 1) {
    if (spl_trie_->is_half_id_yunmu(splid_str[0]) && first.size() > 1) first.resize(1);
    for (LmaNodeLE0 *node : first)
      lma_num = fill_lpi_buffer(lma_buf, max_lma_buf, node, lma_num);
    return lma_num;
  }

  std::vector<LmaNodeGE1*> current, next;
  for (size_t spelling = 1; spelling < splid_str_len; spelling++) {
    id_start = splid_str[spelling];
    id_num = spl_trie_->is_half_id(id_start)
        ? spl_trie_->half_to_full(id_start, &id_start) : 1;
    next.clear();
    const auto append_children = [&](size_t offset, size_t count) {
      for (size_t pos = 0; pos < count; pos++) {
        LmaNodeGE1 *son = nodes_ge1_ + offset + pos;
        if (son->spl_idx >= id_start + id_num) break;
        if (son->spl_idx >= id_start) next.push_back(son);
      }
    };
    if (spelling == 1) {
      for (LmaNodeLE0 *node : first) append_children(node->son_1st_off, node->num_of_son);
    } else {
      for (LmaNodeGE1 *node : current) append_children(get_son_offset(node), node->num_of_son);
    }
    current.swap(next);
    if (current.empty()) return 0;
  }
  for (LmaNodeGE1 *node : current)
    lma_num = fill_lpi_buffer(lma_buf, max_lma_buf, get_homo_idx_buf_offset(node),
                              node, splid_str_len, lma_num);
  return lma_num;
}
