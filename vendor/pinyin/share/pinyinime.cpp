...
extern "C" {
  ...
  // Unexpected Keyboard extension; decoding still uses the same vocabulary.
  void im_set_learning_enabled(bool enabled) {
    if (NULL != matrix_search)
      matrix_search->set_learning_enabled(enabled);
  }
  ...
  void im_reset_search() {
    ...
    // Unexpected Keyboard: reset the public spelling as well as the matrix.
    // The original reset keeps the old bytes, which also breaks prediction.
    matrix_search->search("", 0);
  }
  ...
  float im_get_candidate_score(size_t cand_id) {
    return matrix_search->get_candidate_score(cand_id);
  }

  size_t im_get_unfixed_lemma_count() {
    return matrix_search->get_unfixed_lemma_count();
  }
