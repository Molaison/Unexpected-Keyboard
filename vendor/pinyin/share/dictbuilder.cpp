...
size_t DictBuilder::read_raw_dict(const char* fn_raw,
  ...
  // Unexpected Keyboard: use the declared format capacity, without silently
  // truncating the larger Rime Ice dictionary to the old 240000-entry limit.
  size_t lemma_num = max_item;
  ...
  for (size_t i = 0; i < max_item; i++) {
    ...
    // Unexpected Keyboard: retain Rime's long words (up to kMaxLemmaSize)
    // and low-frequency vocabulary. Filtering is explicit in the converter.
...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  lemma_num_ = read_raw_dict(fn_raw, fn_validhzs, kSysDictIdEnd);
  if (0 == lemma_num_)
    return false;
