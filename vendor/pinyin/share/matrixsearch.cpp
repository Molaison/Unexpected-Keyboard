...
bool MatrixSearch::init(const char *fn_sys_dict, const char *fn_usr_dict) {
  ...
  // Unexpected Keyboard: surface user dictionary failures to the caller.
...
void MatrixSearch::close() {
  // Unexpected Keyboard: initialization can fail before the user dictionary
  // has a filename. Flush only a successfully opened session.
  if (inited_) flush_cache();
  free_resource();
  inited_ = false;
}
...
size_t MatrixSearch::search(const char *py, size_t py_len) {
  ...
  // Unexpected Keyboard: honor the public limit instead of silently dropping
  // input after nine syllables, a restriction of the original phone UI.
...
// Unexpected Keyboard: lower scores mean more probable phrases. This query
// does not choose a candidate or update the user dictionary.
float MatrixSearch::get_candidate_score(size_t cand_id) {
  assert(cand_id < get_candidate_num());
  if (cand_id == 0)
    return mtrx_nd_pool_[matrix_[pys_decoded_len_].mtrx_nd_pos].score;
  float fixed_score = fixed_hzs_ == 0 ? 0 :
      matrix_[spl_start_[fixed_hzs_]].mtrx_nd_fixed->score;
  return fixed_score + lpi_items_[cand_id - 1].psb;
}
  ...
  if (0 == cand_id) {
    ...
    if (1 == lma_id_num_) {
      ...
      if (is_user_lemma(lma_id_[0])) {
        ...
        // that it is hit.
        if (learning_enabled_ && NULL != user_dict_)
...
size_t MatrixSearch::get_lpis(const uint16* splid_str, size_t splid_str_len,
  ...
  if (splid_str_len > 1) {
    // Unexpected Keyboard: keep all candidates when the Rime dictionary
    // leaves too little tail space for the old in-place scratch buffer.
    assert(num <= kMaxLmaPsbItems);
    LmaPsbStrItem *lpsis = lpi_strings_;
    size_t lpsi_num = num;
