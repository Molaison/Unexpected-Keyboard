...
class MatrixSearch {
 private:
  ...
  bool inited_;
  // Unexpected Keyboard: honor IME_FLAG_NO_PERSONALIZED_LEARNING.
  ...
  // Unexpected Keyboard: a large dictionary can fill the candidate array.
  // Deduplication must not borrow space from the unused tail of that array.
  LmaPsbStrItem lpi_strings_[kMaxLmaPsbItems];
  size_t lpi_total_;
 ...
 public:
  ...
  // Unexpected Keyboard: read-only ranking for spelling alternatives.
  float get_candidate_score(size_t cand_id);
  size_t get_unfixed_lemma_count() { return lma_id_num_ - fixed_lmas_; }
